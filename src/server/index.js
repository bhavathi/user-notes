...
let userNotes = [
  ...
  {
    id: 4,
    notes: `The standard chunk of Lorem Ipsum used since the 1500s is reproduced below for those interested. Sections 1.10.32 and 1.10.33 from "de Finibus Bonorum et Malorum" by Cicero are also reproduced in their exact original form, accompanied by English versions from the 1914 translation by H. Rackham.`,
  },
  {
    id: 5,
...
app.post('/api/login', (req, res) => {
  if (req.body.username === 'user@example.com' && req.body.password === '1234') {
    ...
    res.status(403).send({msg: 'Incorrect login!'});
  }
});

app.get('/api/notes', (req, res) => res.send({data: userNotes}));

app.put('/api/notes', (req, res) => {
  if (req.body.id && req.body.notes) {
    const index = userNotes.findIndex( note => note.id === req.body.id);
    userNotes[index].notes = req.body.note;
    res.send('Notes updates successfully');
  } else {
    res.status(500).send({msg: 'Failed to update notes'});
  }
});

app.post('/api/createnotes', (req, res) => {
  console.log('req1221', req.body);
  if(req.body && req.body.notes) {
    const newNotes = {
      id: userNotes.length + 1,
      notes: req.body.notes,
    };
    userNotes.push(newNotes);
    res.send(newNotes);
  } else {
    res.status(500).send({msg: 'Failed to create notes'});
  }
})
